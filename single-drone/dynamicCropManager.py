...
neededItems = { } # needed items for one 'wantedEntity'
...
def one_item_max(wantedEntity): 
	...
	if wantedEntity == Entities.Treasure or not get_cost(wantedEntity) or settings.plantOnlyOneEntity:
		if settings.DEBUG:
			if not get_cost(wantedEntity) or wantedEntity == Entities.Treasure: 
				quick_print("[dynamic_crop_manager] got crop split")
			else: 
				quick_print("[dynamic_crop_manager] got crop split - ('plantOnlyOneEntity')")
				
			quick_print(utils.entityToItem[wantedEntity], ":", get_world_size() * get_world_size())
	...
	farmList = sort_list(farmList, wantedEntity)
